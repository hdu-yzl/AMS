"""Training entry point for the Amazon (3-domain) benchmark.
...
Example:
    python train_amazon.py --model dnn1 --data_dir /path/to/Amazon --cuda 0
"""
...
from modules import mask_models_amazon
from dataloader.loader import AmazonLoader

parser = argparse.ArgumentParser(description="AMS trainer (Amazon)")
parser.add_argument("--dataset", type=str, default="Amazon", help="dataset name")
parser.add_argument("--model", type=str, default="dnn1", help="fusion variant: dnn1-dnn4")

# Dataset information.
parser.add_argument("--data_dir", type=str, default="data/Amazon", help="data directory with TFRecords / npy files")
parser.add_argument("--image_dim", type=int, default=4096, help="image feature dimension")
parser.add_argument("--text_dim", type=int, default=384, help="text feature dimension")
parser.add_argument("--feat_num", type=int, default=1094295, help="total number of id features")
...
parser.add_argument("--save_dir", type=Path, default="save_amazon", help="model checkpoint path")
parser.add_argument("--early_num", type=int, default=1, help="early stop patience")
parser.add_argument("--warmup_rate", type=float, default=0.3, help="fraction of first-epoch steps used to warm up the mask")
parser.add_argument("--batch_num", type=int, default=4864, help="approx. number of training batches per epoch (drives temperature annealing)")
...
parser.add_argument("--thre", type=float, default=0.52, help="mask binarization threshold")
...
# Device information.
parser.add_argument("--cuda", type=int, choices=range(-1, 8), default=0, help="cuda device id, -1 for cpu")
...
class Trainer(object):
    def __init__(self, opt):
        ...
        self.dataloader = AmazonLoader(opt["data_dir"])
        self.device = train_utils.getDevice(opt["cuda"])
        self.network = mask_models_amazon.getModel(opt["model"], opt["model_opt"]).to(self.device)
        ...
        self.optim = mask_models_amazon.getOptim(
    ...
    def train_on_batch(self, label, id_feat, text, image, domain, step):
        ...
        logit1, logit2, logit3, mask = self.network(id_feat, text, image, domain, step)
    ...
    def eval_on_batch(self, id_feat, text, image, domain):
        ...
        with torch.no_grad():
            ...
            logit1, logit2, logit3, mask = self.network(id_feat, text, image, domain, step=1000000)
            logit = (
    ...
    def train(self, epochs):
        ...
        with open(f"amazon_mask_log_{args.model}.txt", "a") as f:
            for d in self.domain_list:
