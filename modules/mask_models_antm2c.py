"""Mask-based multi-modal, multi-domain CTR models for the AntM2C dataset.

AntM2C contains five domains (scenes). The design mirrors the Amazon models:
a per-domain hypernetwork predicts a binary [text, image] mask, and several
fusion variants (DNN1-DNN6) consume the masked modalities. DNN5/DNN6 are
ablation variants that derive soft gates purely from the domain embedding.
"""
...
class LBSign(torch.autograd.Function):
    """Straight-through sign estimator (see ``mask_models_amazon`` for details)."""
...
class DNN1(BasicModel):
    ...
    def __init__(self, opt):
        ...
        self.dnn4 = MultiLayerPerceptron(self.projection_dim * 3, self.embed_dims, dropout, use_bn=use_bn)
        self.dnn5 = MultiLayerPerceptron(self.projection_dim * 3, self.embed_dims, dropout, use_bn=use_bn)
    ...
    def forward(self, id_feat, text, image, d, step=0):
        ...
        logit4 = self.dnn4(x_dnn)
        logit5 = self.dnn5(x_dnn)
        return logit1, logit2, logit3, logit4, logit5, mask
...
class DNN2(BasicModel):
    ...
    def __init__(self, opt):
        ...
        self.dnn4 = MultiLayerPerceptron(self.projection_dim, self.embed_dims, dropout, use_bn=use_bn)
        self.dnn5 = MultiLayerPerceptron(self.projection_dim, self.embed_dims, dropout, use_bn=use_bn)
